...
class GitService {
  ...
  GitService({required this.projectName, required this.repoUrl, required this.token})

  Directory get _projectDir => StorageService.projectDir(projectName)

  Future<ProcessResult> _runGit(List<String> args) async {
    return await Process.run("git", args, workingDirectory: _projectDir.path)
  }
  ...
  Future<String> clone() async {
    ...
    final result = await Process.run("git", ["clone", "--depth", "1", authUrl, _projectDir.path])
  ...
  Future<String> pull() async {
    if (!await _isGitRepo()) return "Not a git repository"
  ...
  Future<String> commitAndPush(String message) async {
    ...
    await _runGit(["config", "user.name", SettingsService.githubUser])
    await _runGit(["config", "user.email", "${SettingsService.githubUser}@opencode.mobile"])
    ...
    if (commitResult.exitCode != 0) {
      final out = (commitResult.stdout as String) + (commitResult.stderr as String)
      if (out.contains("nothing to commit")) return "Nothing to commit"
  ...
  Future<List<String>> getLog({int limit = 5}) async {
    if (!await _isGitRepo()) return ["No commits yet"]
