...
class AgentService {
  ...
  void setGitService(GitService gs) => gitService = gs
...
## API: REST nouns, semantic status codes, {error:{code,message,details}}.

## Workflow
1. Read files before editing.
2. search_code to find patterns. list_files to browse.
3. Write minimal version first. Test happy + error paths.
4. git_sync after each logical unit. Meaningful commit messages.
...
""";
  ...
  Future<String> _executeTool(String name, Map<String, dynamic> args) async {
    try {
      switch (name) {
        case "read_file":
          return await StorageService.readFile(args["project"], args["path"])
        case "write_file":
          await StorageService.writeFile(args["project"], args["path"], args["content"])
          return "File written: ${args["path"]}"
        case "list_files":
          final entries = await StorageService.listDir(args["project"], args["path"] ?? "")
          return entries.map((e) {
            final name = e.uri.pathSegments.last
            return e is Directory ? "📁 $name" : "📄 $name"
          }).join("\n")
        case "delete_file":
          await StorageService.deleteFile(args["project"], args["path"])
        ...
        case "git_sync":
          return await gitService?.commitAndPush(args["message"]) ?? "Git not configured"
        case "git_status":
          return await gitService?.getStatus() ?? "Git not configured"
        default:
    ...
    while (loopCount < maxLoops) {
      ...
      if (msg["tool_calls"] != null && (msg["tool_calls"] as List).isNotEmpty) {
        messages.add(Message(
          role: "assistant",
          content: msg["content"] ?? "",
          toolCalls: (msg["tool_calls"] as List).map((tc) => ToolCall(
            id: tc["id"],
        ...
        for (final tc in msg["tool_calls"]) {
          ...
          try {
            toolArgs = Map<String, dynamic>.from(jsonDecode(fn["arguments"]))
          } catch (_) {
            toolArgs = {}
          }

          yield "\n🔧 $toolName(${fn["arguments"].toString().substring(0, 80 > fn["arguments"].toString().length ? fn["arguments"].toString().length : 80)})\n"
      ...
      messages.add(Message(role: "assistant", content: msg["content"] ?? ""))
      break
    }
  }
