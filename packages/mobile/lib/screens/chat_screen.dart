...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, required this.projectName, required this.gitService})
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _init() async {
    _addMessage("assistant", "Project **${widget.projectName}** ready.\nWhat should we work on?")
  ...
  void _addMessage(String role, String content) {
    setState(() => _messages.add(_ChatMessage(role: role, content: content)))
    _scrollToBottom()
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.projectName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        actions: [
          IconButton(icon: const Icon(Icons.cloud_download), tooltip: "Pull", onPressed: () async {
            final r = await widget.gitService.pull()
            _addMessage("system", r)
          }),
          IconButton(icon: const Icon(Icons.cloud_upload), tooltip: "Push", onPressed: () async {
            final r = await widget.gitService.commitAndPush("Mobile sync")
            _addMessage("system", r)
          }),
        ],
      ),
        ...
        children: [
          ...
          Container(
            decoration: BoxDecoration(
              color: cs.surface,
              border: Border(top: BorderSide(color: cs.outlineVariant ?? const Color(0xFF30363D))),
            ),
    ...
    if (msg.role == "system" || msg.role == "error") {
      return Padding(
        ...
        child: Text(msg.content,
            style: TextStyle(
              fontSize: 12,
              color: msg.role == "error" ? cs.error : cs.onSurfaceVariant,
  ...
  Widget _renderContent(String text, ColorScheme cs, bool isUser) {
    ...
    if (text.contains("```")) {
      ...
      for (final match in regex.allMatches(text)) {
        if (match.start > lastEnd) {
          parts.add(Text(text.substring(lastEnd, match.start), style: TextStyle(color: textColor, fontSize: 14)))
        }
            ...
            children: [
              if (lang.isNotEmpty)
                Padding(
                  ...
                  child: Text(lang, style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
                ),
              SelectableText(code,
      ...
      if (lastEnd < text.length) {
        parts.add(Text(text.substring(lastEnd), style: TextStyle(color: textColor, fontSize: 14)))
      }

      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: parts)
    }

    return SelectableText(text, style: TextStyle(color: textColor, fontSize: 14, height: 1.5))
  }
}
...
class _ChatMessage {
  ...
  _ChatMessage({required this.role, required this.content, this.isStreaming = false})
}
